...
define(["exports", "data", "glMatrix"], function (data, exports) {
  ...
  function updatePoly(array, vertices) {
    const [oldIdx, newIdx] = array;
  ...
  function calcDoubles(list) {
    const doubleVectors = {};

    //clean double vectors
    let i = 0;
    for (let vertex of list) {
      if (doubleVectors[vertex.join(':')]) {
        doubleVectors[vertex.join(':')].push(i);
      } else {
        doubleVectors[vertex.join(':')] = [i];
      }
      i++;
    }
    return Object.values(doubleVectors).filter(b => b.length > 1);
  }
  ...
  function cleanData() {
    ...
    const onlyDoubles = calcDoubles(this.vertices);
    const onlyDoublePolygons = calcDoubles(this.polygonVertices);

    onlyDoubles.forEach(x => {
      this.polygonVertices = updatePoly(x, this.polygonVertices);
    });

    onlyDoublePolygons.forEach(x => {
      const [, ...rest] = x;
      rest.forEach(y => {
        this.polygonVertices.splice(y, 1);
      })
    });
