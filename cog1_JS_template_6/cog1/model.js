...
define(["exports", "dojo", "data", "texture", "glMatrix"], //
    function (exports, dojo, data, texture) {
        ...
        function _initTransformedVertrices() {
            ...
            let i = 0;
            const len = this.modelData.vertices.length;
            for (; i < len; i++) {
        ...
        function _copyVertexDataToTransformedVertrices() {
            ...
            let i = 0;
            const leni = this.modelData.vertices.length;
            for (; i < leni; i++) {
                // Speedup vars to save indices.
                const vertex = this.modelData.vertices[i];
                const transformedVertex = this.transformedVertices[i];
                let j = 0;
                const lenj = vertex.length;
                for (; j < lenj; j++) {
        ...
        function applyMatrixToNormals(matrix) {
            ...
            const vertexNormals = this.modelData.vertexNormals;
            const polygonNormals = this.modelData.polygonNormals;
