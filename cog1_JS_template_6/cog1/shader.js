...
define(["exports"], function (exports) {
    ...
    const scanlineInterpolationVertices = [];
    // Vertex vec3 interpolated on scanline.
    const scanlineInterpolationVertex = [];
    const scanlineInterpolationVertexDelta = [];
    //
    // For toon shading.
    // To form zones we need the intensity range.
    // Calculated in setLights function.
    let sumOfAllLightTypesIntensity = 0;
    ...
    function setLights(ambientLI, pointLI, pointPos, specularLI, specularLIExpo) {
        ...
        if (pointPos) {
            for (let i = 0; i < pointLightPosition.length; i++) {
     ...
     */
    const toonInit = phongInit;
    const toonInterpolationPrepareScanline = phongInterpolationPrepareScanline;
    const toonInterpolationStepOnScanline = phongInterpolationStepOnScanline;
