...
define(["exports", "scene"], function (exports, scene) {
    ...
    function init(_ctx, _bgColor) {
        ...
        if (width != imageData.width || height != imageData.height) {
            console.log(
                "WARNING: Dimension of the canvas pixel match the CSS pixel.",
            );
        }
    ...
    function reset() {
        ...
        if (dirtyEndIndex > dirtyStartIndex) {
            ...
            // Cut rectangles  as views fitting the dirtyRect.
            dirtyData = new Uint8ClampedArray(
                framebuffer.buffer,
                dirtyStartIndex,
                dirtyWidth,
            );
            dirtyDataReset = new Uint8ClampedArray(
                resetBuffer32.buffer,
                0,
                dirtyWidth,
            );
            dirtyData.set(dirtyDataReset);
    ...
    function scaleZBuffer() {
        ...
        // Get min and max.
        for (var i = 0; i < zBufSize; i++) {
            if (zBuf[i] == maxDistance) continue;
            if (zBuf[i] > max) {
                max = zBuf[i];
            } else if (zBuf[i] < min) {
                min = zBuf[i];
    ...
    function MultiplyFramebufferWithZBuffer(greyOnly) {
        ...
        const dirtyStartIndex = dirtyRect.y * width + dirtyRect.x;
        const dirtyEndIndex = dirtyRect.yMax * width + dirtyRect.xMax;
