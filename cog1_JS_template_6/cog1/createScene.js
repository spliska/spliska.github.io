...
define(["exports", "scenegraph", "animation"], function (
     ...
     */
    function init() {
        const cubeNode = scenegraph.createNodeWithModel("myModel", "myModel");
        ...
        const cubeNode1 = scenegraph.createNodeWithModel("cube 1", "cube", {
            scale: 70,
            textureURL: "stoneWall.jpg",
        });
        cubeNode1.translate([50, 200, 0]);
        //cubeNode1.rotate([1,1,1]);
        const cubeNode2 = scenegraph.createNodeWithModel("cube 2", "cube", {
            scale: 50,
            textureURL: "uvTest.jpg",
        });
        cubeNode2.translate([-100, -400, 0]);
        //cubeNode2.rotate([-1,-1,-1]);
        const cubeNode3 = scenegraph.createNodeWithModel(
            "cube procedural texture",
            "cube",
            {scale: 50, textureURL: "sampleProceduralRGB"},
        );
        const cubeNode4 = scenegraph.createNodeWithModel(
            "cube six faces texture",
            "cube",
            {scale: 200, textureURL: "OrbitCube.gif", sixFacesTexture: true},
        );
        const cubeNode5 = scenegraph.createNodeWithModel("cube 3x3 texture", "cube", {
            scale: 50,
            textureURL: "cubeColor.png",
            sixFacesTexture: true,
        });
        const cubeNode6 = scenegraph.createNodeWithModel(
            "cube Escher texture",
            "cube",
            {scale: 200, textureURL: "EscherCubeFish.gif", sixFacesTexture: true},
        );

        const insideOutPolyNode = scenegraph.createNodeWithModel(
            "insideOutPoly",
            "insideOutPoly",
        );

        const diamondNode = scenegraph.createNodeWithModel("diamond", "diamond");

        const torusNode = scenegraph.createNodeWithModel("torus", "torus");
        const torusNode1 = scenegraph.createNodeWithModel("torus 13", "torus", {
            r2: 50,
            n2: 13,
            color: 8,
        });

        const teapotNode = scenegraph.createNodeWithModel("teapot", "teapot", {
            color: 0,
            scale: 40,
        });
        const dirtyTeapotNode = scenegraph.createNodeWithModel(
            "dirtyTeapot",
            "teapot_dirty",
            {color: 8},
        );
        ...
        const waltheadNode = scenegraph.createNodeWithModel("walthead", "walthead", {
            color: 8,
        });

        const plainNode1 = scenegraph.createNodeWithModel("plain", "plain", {
            scale: 200,
            color: 9,
            textureURL: "land_ocean_ice_2048.jpg",
        });

        const emptyNode1 = scenegraph.createNodeWithModel("empty", "empty");
        ...
        const lightnode = scenegraph.createPointLightNode("light", "diamond");
