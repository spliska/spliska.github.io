...
define([
    ...
    const projection = mat4.identity(mat4.create());
    // The projection type can be set from the UI, see setProjectionType.
    let projectionType = "ortho";
    const projectionTypes = ["ortho", "frustum", "perspective"];
    // Viewport transformation matrix.
    const viewport = mat4.identity(mat4.create());
    // Combined matrix for faster calculation.
    const viewportProjection = mat4.create();
    ...
    function initPipeline() {
        ...
        const rgb = bgColorStyleCanvas.match(/rgb\((\d+),\s(\d+),\s(\d+)\)/);
    ...
    function render() {
        ...
        let foundInteractiveNode = false;
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            const worldModelview = nodes[i].updateModelview();
            ...
            const worldRotation = nodes[i].updateRotation();
        ...
        if (shader.usesLightLocation()) {
            //&& ! scenegraph.getPointLightNode()) {
    ...
    function renderModel(model) {
        ...
        const vertices = model.getProjectedVertices();
        const polygons = modelData.polygonVertices;
        const polygonNormals = model.getTransformedPolygonNormals();
        const textureCoord = modelData.textureCoord;
        const texture = texturing ? model.getTexture() : null;
        ...
        for (let p = 0; p < polygons.length; p++) {
            ...
            const normal = polygonNormals[p];
            const color = modelData.colors[modelData.polygonColors[p]];
    ...
    function renderModelNormals(model) {
        ...
        const vertices = model.getProjectedVertices();
        const polygons = modelData.polygonVertices;
        const vertexNormals = model.getTransformedVertexNormals();
        const polygonNormals = model.getTransformedPolygonNormals();
        ...
        const perspectiveNormal = [0, 0, 0, 0];
    ...
    function clearLightLocationMarker() {
        ...
        const x = posLightLocationMarker[0];
        const y = posLightLocationMarker[1];
    ...
    function drawLightLocationMarker() {
        ...
        const x = posLightLocationMarker[0];
        const y = posLightLocationMarker[1];
        const z = posLightLocationMarker[2];
        // The size/radius depends on z position of light.
        // Assume z in [-500,+500]
        const r = Math.floor((z + 500) / 100) + 5;
        const quater = Math.PI * 0.5;
        ctx.fillStyle = "grey";
        // Background circle.
        ctx.beginPath();
    ...
    function drawGrid(xy, xz, yz) {
        const range = 500;
        const step = 10;
        const color = data.getColorByName("grey");
        let i;
        const drawLine = function (varIndex, constIndex) {
        ...
        for (i = -range; i <= range; i += step) {
            for (let j = -range; j <= range; j += range) {
                if (xy) {
                    drawLine(0, 1);
                    drawLine(1, 0);
                }
    ...
    function displayInfoForNode(node) {
        let str = "interactive node";

        if (!node.isVisible()) {
            str += " is not visible";
        }
        str += ": " + node.name;
        const texture = scenegraph.getTextureNameForNode(node);
        if (texture) {
            str += " (" + texture + ")";
        }
    ...
    function displayRenderStatistics(startDate) {
        ...
        const frameDuration = curDate - startDate;
        let curFramerate;
        if (!frameDuration) {
            curFramerate = 0;
        } else {
        ...
        let str =
            "frames: " + curFramerate.toFixed(1) + " <" + framerate.toFixed(1) + ">";
        //console.log(str);
    ...
    function displayText(text, xOffset, yOffset, clearfirst) {
        ...
        // Clear first.
        if (clearfirst) {
            const clearWidth = text.length * fontLineHeightInPt * 0.6;
