...
define(["exports", "data"], function (exports, data) {
    ...
    exports.create = function (parameter) {
		...
		const schenkel = 20; // Seite 1 und Seite 2
        //var spitzenwinkel = 36;
		const spitzenwinkelInRad = 36 * (Math.PI / 180);
		//var aussenSchenkelWinkel = 54;
		const aussenSchenkelWinkelInRad = 54 * (Math.PI / 180);

		const seite3 = Math.sqrt(2 * schenkel * schenkel * (1 - Math.cos(spitzenwinkelInRad)));
		const seite3halbe = seite3 / 2;

		//var schenkelWinkel = 72;
		const schenkelWinkelInRad = 72 * (Math.PI / 180);
		const hoeheDreieck = Math.sin(schenkelWinkelInRad) * schenkel;

		// Berechnung der fehlenden Werte der Ankatheten und Gegenkatheten
		const x1 = Math.sin(aussenSchenkelWinkelInRad) * schenkel; // Hypotenuse = gleichseitige Schenkel
		const y1 = Math.cos(aussenSchenkelWinkelInRad) * schenkel;
		const x2 = Math.sin(aussenSchenkelWinkelInRad) * seite3; // Hypotenuse = 3. Seite bzw. Grundlinie
		const y2 = Math.cos(aussenSchenkelWinkelInRad) * seite3;

		const obereLinieY = y1 + y2 + x2; // hier liegen Punkte 4, 5, 7 und 8
