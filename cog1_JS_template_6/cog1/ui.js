...
// Local parameters for required modules.
    function ui(exports, app, layout, scene, scenegraph, animation, shader, dojo, sniff, html, on, dom, domConstruct, domStyle, domProp, mouse) {
        ...
        const lightSliders = [];
        let ambientLightIntensitySlider;
        let pointLightIntensitySlider;
        let specularLightIntensitySlider;
        let specularLightExponentSlider;
        ...
        function postSceneInit(width, height) {
            ...
            const max = [width, height, 500];
            const lightPos = shader.getLightPosition();
            //console.log("max"+max);
        ...
        function debugButtonCallback() {
            const fkt = shader.getShadingFunction();
            console.log(fkt);
        }
        ...
        function initDisplayButtons() {
            ...
            const button = createButton("fullscreen", callback, false, headerContainer, "floatRight");
        ...
        function initTransformationButtons() {
            ...
            for (let transform in delta) {
                label(transform + ":");
                br();
                for (let axis in axises) {
        ...
        function initEffektButtons() {
            ...
            createButton("clean data", function () {
                const interactiveNode = scenegraph.getInteractiveNode();
                if (interactiveNode) {
                    const model = interactiveNode.getModel();
                    if (model) {
                        model.cleanData();
                    }
                }
        ...
        function initProjectionControls() {
            ...
            label("projection: ");
            br();
            const currentProjectionType = scene.getProjectionType();
            scene.projectionTypes.forEach(function (name) {
                const checked = scene.isProjectionType(name);
                const callback = (function (_name) {
                    return function () {
        ...
        function initShaderControls() {
            ...
            label("shader: ");
            br();
            const currentShadingFunctionName = shader.getShadingFunctionName();
            for (let i = 0; i < shader.shadingFunctionNames.length; i++) {
                const name = shader.shadingFunctionNames[i];
                const checked = currentShadingFunctionName == name ? true : false;
                const callback = (function (_name) {
                    return function () {
        ...
        function initScenegraphButtons() {
            ...
            br();
            const nodeNames = scenegraph.getNodeNames();
            for (let n in nodeNames) {
                const name = nodeNames[n];
                ...
                const animStr = animation.getAnimationsForNodeByName(name);
                if (animStr) {
                    text(" (" + animStr + ")");
                }
                // Show the parent
                const parentName = scenegraph.getParentNameOfNodeByName(name);
                if (parentName) {
                    text(" -> " + parentName);
                }
                br();
        ...
        function initLightControls() {
            ...
            for (let i = 0; i < 3; i++) {
                const dirctions = ["X", "Y", "Z"];
                const labelText = "point light pos " + dirctions[i] + ": ";
                const sliderCallbackFct = (function (dim) {
                    // Set the initial values of the slider to 0.
                    return function (value) {
                        const pos = [undefined, undefined, undefined];
                        pos[dim] = value;
        ...
        function createSlider(val, min, max, steps, labelText, onChangeFct, floatValues) {
            p();
            const idStringPostfix = labelText.replace(/\s/g, "_");
            const sliderNode = domConstruct.create('div', {
                id: "sliderNode" + idStringPostfix
            }, controlsContainer);
            label(labelText, sliderNode);
            const valX = text(val.toString(2), sliderNode);
            const slider = new dijit.form.HorizontalSlider({
        ...
        function createToggleButton(label, onClickFct, checked, _br, container) {
            ...
            const button = new dijit.form.ToggleButton({
        ...
        function createRadioButton(groupname, _label, onClickFct, checked, _br, container) {
            ...
            const button = new dijit.form.RadioButton({
                //iconClass : "dijitCheckBoxIcon",
                checked: checked,
                value: _label,
                name: groupname,
        ...
        function createCheckBox(_label, onClickFct, checked, _br, container, _disabled) {
            ...
            const button = new dijit.form.CheckBox({
                //iconClass : "dijitCheckBoxIcon",
                checked: checked,
                disabled: _disabled,
                value: _label,
        ...
        function text(_text, container) {
            ...
            const span = domConstruct.create("span", {}, container);
            html.set(span, _text);
            return span;
        }
        ...
        function handleKeyEvent(e) {
            ...
            const callback = keyCallbacks[modifierKey][key];
        ...
        function initMouseEvents() {
            const canvasDiv = layout.getCanvasContainer();
            //dom.byId("canvasDiv");
        ...
        function onMouseScroll(e) {
            ...
            const scroll = e[(!sniff("mozilla") ? "wheelDelta" : "detail")] * (!sniff("mozilla") ? 1 : -1);
            //console.log(scroll);
            const scaledAxis = [1, 1, 1];
            var sign = "+";
            if (scroll < 0) {
                var sign = "-";
            }
            const callback = callbackFactoryTransformNode("scale", sign, delta["scale"], [1, 1, 1]);
            callback();
        }
        ...
        function onMouseMove(e) {
            ...
            // Calculate movement.
            const mousePosXNew = e.clientX;
            const mousePosYNew = e.clientY;
            // currMouseButton = e.button;
            let diffX = mousePosXNew - mousePosX;
            let diffY = mousePosYNew - mousePosY;
            mousePosX = mousePosXNew;
            mousePosY = mousePosYNew;

            // Some representation of Z dim with a 2D mouse :(
            let diffZ = (diffX + diffY) / (Math.abs(diffX - diffY) + 1.0);
