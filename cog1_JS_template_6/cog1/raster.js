...
define(["exports", "shader", "framebuffer", "data", "glMatrix"], function (
    ...
    function scanlineFillPolygon(
        ...
        for (let y = 0; y < height; y++) {
            ...
            //if ((scanlineIntersection.length < 2) || (scanlineIntersection.length % 2)) {
            //    console.log("Error in number of intersection (" + scanlineIntersection.length + ") in line: " + y);
            //}
            ...
            for (let ix = 0; ix < scanlineIntersection[y].length - 1; ix += 2) {
                // Calculate interpolation variables for current scanline.
                // Necessary for z-buffer, shading and texturing.
                const x1 = scanlineIntersection[y][ix];
                const x2 = scanlineIntersection[y][ix + 1];

                // Fill line section inside polygon, loop x.
                for (let x = x1.x; x <= x2.x; x++) {
                    framebuffer.set(x, y, getZ(x, y), color);
                }
            }
            // Set z shorthand.
    ...
    function calcPlaneEquation(vertices, polygon, normal) {
        ...
        const p = polygon[0];
        const x = vertices[p][0];
        const y = vertices[p][1];
        const z = vertices[p][2];
        D = -(A * x + B * y + C * z);
